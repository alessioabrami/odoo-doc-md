# Aggiungi al carrello — Odoo 18.0 documentazione
...
Hiding the Add to Cart button is often used by B2B eCommerces that need to restrict purchases only to [customers with an account](checkout.html#ecommerce-checkout-policy), but still want to display an online product catalog for those without.
...
You can enable the Buy Now button to take customers directly to the [review order](checkout.html#ecommerce-checkout-review-order) step instead of adding the product to the cart. This button is an _additional_ option and does not replace the Add to Cart button. To do so, go to Website ‣ Configuration ‣ Settings, scroll to the Shop—Checkout Process section, enable Buy Now, and Save.
