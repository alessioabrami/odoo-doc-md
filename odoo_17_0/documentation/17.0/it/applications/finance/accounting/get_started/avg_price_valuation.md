# Prezzo medio sulla merce resa — Odoo 17.0 documentazione
...
Al momento della ricezione del prodotto, Odoo assicura che le aziende possano pagare i beni acquistati spostando, in via preventiva, un importo corrispondente al prezzo dei beni ricevuti nel [conto passivo](cheat_sheet.html), **Inserimento scorte**. Successivamente, una volta ricevuta la fattura, l’importo nel conto deposito viene trasferito nel _Conto di debito_. I trasferimenti in questo conto indicano che la fattura è stata pagata. Il conto **inserimento scorte** viene riconciliato al momento della ricezione della fattura fornitore.

La valutazione di magazzino è un metodo per calcolare quanto vale internamente ogni prodotto nelle scorte. Dato che c’è una differenza tra il prezzo di **valutazione** del prodotto e l’effettivo prezzo di **acquisto** del prodotto stesso, il conto **Valutazione magazzino** non è collegato alle operazione di credito e debito del conto **Inserimento scorte**.
...
[ __Edit on GitHub](https://github.com/odoo/documentation/edit/17.0/content/applications/finance/accounting/get_started/avg_price_valuation.rst)
